...
def _draw_text(image, text, position, color, scale = 1.0, offset_lines = 0):
  ...
  text_image = Image.new(mode = "RGBA", size = (100,20), color = (0, 0, 0, 0))
...
def show_detections_API(output_path, show_image, image, scored_boxes_by_class_index, class_index_to_name):
  # Draw all results
  ctx = ImageDraw.Draw(image, mode = "RGBA")
  color_idx = 0
  for class_index, scored_boxes in scored_boxes_by_class_index.items():
