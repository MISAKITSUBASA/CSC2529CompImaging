...
def predict_one_API(model, frame, show_image, output_path):
  from .datasets import image
  hdred_image,image_data, image_obj, _, _ = image.load_image_API(image=frame, preprocessing = model.backbone.image_preprocessing_params, min_dimension_pixels = 600)
  labled_frame = predict_API(hdred_image=hdred_image, model = model, image_data = image_data, image = hdred_image, show_image = show_image, output_path = output_path)
  return labled_frame

def predict_API(hdred_image,model, image_data, image, show_image, output_path):
  image_data = t.from_numpy(image_data).unsqueeze(dim = 0).cuda()
  scored_boxes_by_class_index = model.predict(image_data = image_data, score_threshold = 0.7)
  labled_frame = visualize.show_detections_API(
    output_path = output_path,
    show_image = show_image,
    image = image,
    scored_boxes_by_class_index = scored_boxes_by_class_index,
